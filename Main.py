...
def main():
    ...
    '''
    You can set vm_file_path to be either a folder (and then an asm file with 
    the folder name will be created in the folder) or set it to be a vm file
    (and then an asm file with the file name will be created in the same location)    
    '''
    ...
    if not vm_file_path.endswith(".vm"):
        ...
        cw = CodeWriter(os.path.join(vm_file_path, "{0}.asm".format(vm_file_path.split(os.sep)[-2])))
    else:
